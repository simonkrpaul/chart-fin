...
function parseTimestamp(raw: string | number | undefined | null): number | null {
  ...
  if (!isNaN(n)) {
    ...
    //
    // Ranges:
    //   >= 1e11  → already ms (13-digit), use as-is
    //   >= 1e9   → Unix seconds (10-digit), multiply by 1_000
    //   >= 1e6   → truncated seconds (7-8 digit, e.g. Bybit CSV export), multiply by 1_000_000
    //   < 1e6    → not a valid epoch
    if (n >= 1e11) return n;
    if (n >= 1e9)  return n * 1000;
    if (n >= 1e6)  return n * 1000000;
    return null;
  }
